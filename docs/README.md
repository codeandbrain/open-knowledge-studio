...
| Feature | v1 | v2 |
|---------|----|----|
| Storage | localStorage (5-10 MB) | **IndexedDB (GB-scale)** |
| Search | None | **Client-side fuzzy full-text search** |
| Voice Input | None | **Web Speech API dictation** |
| Offline | Partial | **Full PWA with Service Worker** |
| Document Versions | None | **Auto-save every 30s + manual snapshots** |
| Templates | Static | **Dynamic template gallery with inject** |
| LLM Providers | Gemini only | **Gemini, OpenAI, Anthropic, DeepSeek, Groq, Ollama** |
| Gemini Model | 2.0-flash (deprecated) | **3.5-flash (stable)** |
| Theme | Dark only | **Dark + Light mode** |
| File Types | Basic | **Drag-and-drop, 9+ formats** |
| Export | Limited | **Markdown, HTML, JSON, bulk export/import** |
